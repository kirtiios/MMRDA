...
@objc open class ACFloatingTextfield: UITextField {
    ...
    fileprivate var passwordShowButton : UIButton?
    ...
    @IBInspectable open var isSecure : Bool = false {
        didSet{
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        if isSecure == true {
            ...
            return CGRect(x:4, y:4, width:bounds.size.width - 60, height:bounds.size.height);
        }else{
...
fileprivate extension ACFloatingTextfield {
    ...
    func addPasswordShowButton() {
        //Bottom Line UIView Configuration.
        if passwordShowButton?.superview != nil {
            return
        }
        
        passwordShowButton = UIButton(frame: CGRect(x:0, y:self.frame.origin.y, width:60, height:self.frame.size.height))
        
        passwordShowButton?.setImage(UIImage.init(named: "passwordHide"), for: .normal)
        passwordShowButton?.backgroundColor = .clear
        passwordShowButton?.translatesAutoresizingMaskIntoConstraints = false
        passwordShowButton?.addTarget(self, action: #selector(showHidePassword), for: .touchUpInside)
        self.addSubview(passwordShowButton!)
        
        let leadingConstraint = NSLayoutConstraint.init(item: passwordShowButton!, attribute: .leading, relatedBy: .equal, toItem: self, attribute: .trailing, multiplier: 1, constant: -60)
        let trailingConstraint = NSLayoutConstraint.init(item: passwordShowButton!, attribute: .trailing, relatedBy: .equal, toItem: self, attribute: .trailing, multiplier: 1, constant: 0)
        let bottomConstraint = NSLayoutConstraint.init(item: passwordShowButton!, attribute: .bottom, relatedBy: .equal, toItem: self, attribute: .bottom, multiplier: 1, constant: -10)
        if #available(iOS 9.0, *) {
            let widthConstraint = passwordShowButton?.widthAnchor.constraint(equalToConstant: 60)
            passwordShowButton?.addConstraint(widthConstraint!)
        } else {
    ...
    func addFloatingLabel(){
        ...
        labelPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "Roboto")!, size: 12)
