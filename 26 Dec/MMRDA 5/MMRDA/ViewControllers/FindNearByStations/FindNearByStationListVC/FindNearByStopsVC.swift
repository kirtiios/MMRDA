...
let APP_BLUE_COLOR = UIColor(hexString: "#00397E")
...
//        self.callBarButtonForHome(isloggedIn:true,leftBarLabelName:"findnearbybusstops".LocalizedString, isHomeScreen:false,isDisplaySOS: false)
       ...
       // actionTransportMediaChange(btnMetro)
        ...
        btnBus.setImage(UIImage(named:"TrasperantMBMCImage"), for:.normal)
        btnBus.setImage(UIImage(named:"MBMCImage"), for:.selected)
    ...
    func getNearByStop(objStation:FareStationListModel?,location:CLLocationCoordinate2D){
        ...
        param["intTransportModeID"] = TrasperModeID
        param["decCurrentLat"] = location.latitude//LocationManager.sharedInstance.currentLocation.coordinate.latitude
        param["decCurrentLong"] = location.longitude//LocationManager.sharedInstance.currentLocation.coordinate.longitude
        activityIndicator.isHidden = false
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            self.objViewModel.getfindNearByStop(param: param)
            
            if objStation == nil {
                self.cirlce?.position = CLLocationCoordinate2D(latitude: LocationManager.sharedInstance.currentLocation.coordinate.latitude, longitude: LocationManager.sharedInstance.currentLocation.coordinate.longitude)
//                self.isSearchActive = false
//                self.txtSearchBar.text = nil
//                self.arrAllStationList.removeAll()
//                self.tblView.reloadData()
//                self.isPolilyneDraw = false
//                self.getRefreshData()
            }else{
    ...
    @IBAction func btnDownArrowClick(_ sender: UIButton) {
        if imgDownArrow.image == UIImage(named: "downArrow"){
    ...
    @IBAction func actionTransportMediaChange(_ sender: UIButton) {
        ...
        param["UserID"] = Helper.shared.objloginData?.intUserID
        param["decStationLat"] =  "0"
        param["decStationLong"] = "0" //72.85872096902258
        param["strStationName"] = ""
        param["intTransportModeID"] = TrasperModeID
        param["decCurrentLat"] = LocationManager.sharedInstance.currentLocation.coordinate.latitude
        param["decCurrentLong"] = LocationManager.sharedInstance.currentLocation.coordinate.longitude
        param["strlanguage"] = Helper.shared.getLanguageCodeforApi()
        
        activityIndicator.isHidden = false
        self.objViewModel.getfindNearByStop(param: param)
//        self.objViewModel.bindDirectionDataData = { responseDict in
//            self.isPolilyneDraw = true
...
//            }
//        }
//        self .getNearByStop(objStation: arrSearchStationList[0], location: LocationManager.sharedInstance.currentLocation.coordinate)
    }
...
extension FindNearByStopsVC :UITableViewDelegate,UITableViewDataSource {
    ...
    @objc func btnActionDirectionClicked(sender:UIButton){
        ...
        param["decCurrentLat"] =  LocationManager.sharedInstance.currentLocation.coordinate.latitude
        param["decCurrentLong"] = LocationManager.sharedInstance.currentLocation.coordinate.longitude
        param["intTransportModeID"] = busttype.rawValue // kirti 
//        1 for driving
//        2 for walking
...
extension FindNearByStopsVC {
    ...
    func getPinImage(typeID:Int)->UIImage{
        if typeID == transPortType.Metro.rawValue {
            ...
            return UIImage(named: "newpinMetro") ?? UIImage()
        }
...
//            param["strStationName"] = ""
//            param["intTransportModeID"] = TrasperModeID
//            param["decCurrentLat"] = position.target.latitude
//            param["decCurrentLong"] = position.target.longitude
//            param["strlanguage"] = Helper.shared.getLanguageCodeforApi()
//            
//            self.arrAllStationList.removeAll()
//            activityIndicator.isHidden = false
