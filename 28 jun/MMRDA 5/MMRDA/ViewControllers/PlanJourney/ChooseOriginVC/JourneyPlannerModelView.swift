...
class JourneyPlannerModelView {
    ...
    func getGetMetroTripDirectionJourneyPlanner(param:[String:Any],completionHandler:@escaping ([String:Any]?)->Void?){
        
        ApiRequest.shared.requestPostMethod(strurl: apiName.GetMetroTripDirection, params: param, showProgress: true, completion: { suces, data, error in
            do {
                if suces {
                    let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String:Any]
                    completionHandler(json)
