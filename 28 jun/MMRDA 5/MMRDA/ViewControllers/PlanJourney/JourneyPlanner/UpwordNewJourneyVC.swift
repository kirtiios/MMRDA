...
//        return arrUpword.transitPathsReturnUpwardTrip?.count
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if downward == true{
            ...
            let formattedDateDeparture = formattedDateString(from: fromStationDepartureDate, inputFormat: "MM/dd/yyyy HH:mm:ss", outputFormat: "h:mm a")
...
//        if downward == true{
        ...
        print(PassThellData)
        if downward == true{
            vc.objJourney = PassThellData?.downwardTrip?[0]
            vc.PassFullnameFrom = FullnameTo
            vc.PassFullnameTo = FullnameFrom
        }else{
            vc.objJourney = PassThellData?.upwardTrip?[0]
