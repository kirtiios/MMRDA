...
//                    self.btnBookTicket.isHidden = true
    ...
    @IBAction func btnActionBuyClicked(_ sender: UIButton) {
        ...
        param["isReturnJourney"] = true
        param["decCyclingRange"] = 0
        param["decWalkingRange"] = 0
        param["isArrivalBased"] = false
        param["isAutoTaxiTransport"] = false
        param["isBusTransport"] = false
        param["isCyclingTransport"] = false
        param["isMetroTransport"] = false
        param["strHoltTime"] = ""
        param["strSelectedTime"] = ""
        ...
        objViewModel.getJourneyPlanner(param: param) { array in
            if array?.count == 1 && array?.first?.upwardTrip?[0].transitPaths?.count ?? 0 < 1 {
