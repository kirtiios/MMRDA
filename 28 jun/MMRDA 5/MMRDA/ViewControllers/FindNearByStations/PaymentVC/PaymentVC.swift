...
//        self.btnDistance.setTitle("\(self.objToStation?.km ?? 0) KM", for: .normal)
//        self.btnDistance.setTitle("\(self.objJourney?.journeyPlannerStationDetail?.km ?? 0) KM", for: .normal)
        objViewModel.getFareCalculator(fromStationID:fromStationCode ?? "" , toStationID:toStationid,isPenality: isPenalty, intTicketCode: passintTicketCode ?? 0) { [self] faremodel in
            ...
            self.fare = Double(faremodel?.discountedFare ?? 0)
        ...
        dropDown.selectionAction = { [weak self] (index: Int, item: String) in
          print("Selected item: \(item) at index: \(index)")
           ...
           // self?.btnTotalAmount .setTitle("Rs.\(num2)", for: .normal)
            self?.getFareCalculatore(isPenalty: true,toStationid:"\(self?.objPenaltyData?.intStationID ?? 0)")
            self?.tblview.reloadData()
...
//                }
            ...
            let num1 = Double(self.btnNoOfPassengers.currentTitle ?? "0") ?? 0 //Double(cell.lblAdultCount.text ?? "0") ?? 0
            if fromType == .NearByStop {
//                fare = Double(objFareCal?.baseFare ?? 0)
                fare = Double(objFareCal?.discountedFare ?? 0)
...
//                fare = Double(objFareCal?.baseFare ?? 0)
                ...
                if btnTotalAmount.currentTitle == "" || btnTotalAmount.currentTitle == nil{
                    fare = 0.0
                }else{
...
//
//
//                    cell.lblAmountinRS.text = "Rs.\(fare)"
//                }
