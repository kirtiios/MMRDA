...
class PaymentVC: BaseVC {
    ...
    var getUpwordValue:JourneyPlannerStationDetail?
    var getDownwordValue:JourneyPlannerStationDetail?
        ...
        dropDown.selectionAction = { [weak self] (index: Int, item: String) in
          print("Selected item: \(item) at index: \(index)")
            ...
            if self?.isfromUpwoedNewJourneyVcPass == true{
                let num3 = Int(self?.downfare ?? 0)  * (Int(item) ?? 0)
                print(num3)
            }
...
//                param["intRouteID"] = self.objJourney?.transitPaths?.first?.routeid
                ...
                param["intDisscount"] =  0//Int(self.objJourney?.journeyPlannerStationDetail?.fare ?? 0)
                param["intBasicFare"] = basicRate
                param["intTicketCode"] =  self.passintTicketCode
            ...
            if fromType == .QRCodePenalty {
                ...
                param["strCategoryWiseJSON"] = [["intBasicFare":basicRate,"intCategoryID":1,"intTotalFare":total,"intTotalQty":self.btnNoOfPassengers.title(for:.normal) ?? "0"]]
                if isfromSuggestedItineratyVCPass{
    ...
    //                                                  "intTransportModeID":0,
                                                      ...
                                                      "bReturnJourney":objJourney?.journeyPlannerStationDetail?.transportWiseFares?.first?.bReturnJourney]] ////trasportid
                }else{
                  
                    param["strTransportWiseJSON"] = [["decKM":getUpwordValue?.km, //*
                                                      "intBasicFare":basicRate ,
                                                      "intFromStationID": param["intFromStationID"],
                                                      "intRouteID":param["intRouteID"],
                                                      "intServiceTypeID":getUpwordValue?.transportWiseFares?.first?.intServiceTypeID,
                                                      "intToStationID":param["intToStationID"],
                                                      "intTotalFare":total,
                                                      "intTotalQty":numberQty ,
                                                      "intTransportModeID":1,
                                                      "strLine":getUpwordValue?.transportWiseFares?.first?.schNo,//strLineNumber ,
    //                                                  "intTransportModeID":0,
                                                      "strTransportNumber" : 0,
                                                      "bReturnJourney":getUpwordValue?.transportWiseFares?.first?.bReturnJourney],
                                                     ["decKM":getDownwordValue?.km, //*
                                                                                       "intBasicFare":basicRate ,
                                                                                       "intFromStationID": param["intFromStationID"],
                                                                                       "intRouteID":param["intRouteID"],
                                                                                       "intServiceTypeID":getDownwordValue?.transportWiseFares?.first?.intServiceTypeID,
                                                                                       "intToStationID":param["intToStationID"],
                                                                                       "intTotalFare":total,
                                                                                       "intTotalQty":numberQty ,
                                                                                       "intTransportModeID":1,"strLine":getDownwordValue?.transportWiseFares?.first?.schNo,//strLineNumber ,
                                     //                                                  "intTransportModeID":0,
                                                      "strTransportNumber":0,"bReturnJourney":getDownwordValue?.transportWiseFares?.first?.bReturnJourney]]
                }
            }
//            strTransportNumber
//            bReturnJourney
