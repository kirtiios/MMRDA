...
class FareCalViewModel {
    ...
    func getFareCalculator(completion:@escaping(FareCalResponseModel?)->Void?, intTicketCodePass :Int){
        ...
        if objFromFareStation == nil && objTOFareStation == nil {
            ...
            param["intTicketCode"] = intTicketCodePass//118
            param["intQty"] = 1
...
//    func getJourneyPlanner(param:[String:Any],completionHandler:@escaping ([ReturnJourneyPlannerApiModel]?)->Void?) {
...
//        })
//    }
    func getJourneyPlanner(param:[String:Any],completionHandler:@escaping ([ReturnJourneyPlannerApiModelData]?)->Void?) {
        ApiRequest.shared.requestPostMethod(strurl: apiName.GetReturnJourneyPlan, params: param, showProgress: true, completion: { suces, data, error in
            if var obj = try? JSONDecoder().decode(ReturnJourneyPlannerApiModel.self, from: data) {
                if obj.issuccess ?? false {
                    completionHandler(obj.data)
