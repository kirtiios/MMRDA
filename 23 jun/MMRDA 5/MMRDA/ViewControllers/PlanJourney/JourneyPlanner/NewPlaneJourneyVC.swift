    ...
    @IBAction func btnActionClicked(_ sender: UIButton) {
        ...
        if sender == btnSearch {
            ...
            if objFrom == nil   {
                ...
                print("select the value:-\(self.cycleKM)........\(self.walkKM)")
                newApiviewModel.getReturnJourneyPlanner(param: param) { newApiData in
                    if self.isReturnJourney == false{
                        if newApiData?.first?.upwardTrip?[0].transitPaths?.count ?? 0 <= 0{
                            self.showAlertViewWithMessage("APPTITLE".LocalizedString, message: "txtNoTrip".LocalizedString)
                        }else{
                            let vc =  UIStoryboard.SuggestedItineraryVC()
                            vc.upwardRoundTrip = newApiData?.first?.upwardTrip?[0].transitPaths as? [TransitPaths]
                            vc.upwardRoundJourney = newApiData?.first?.upwardTrip?[0].journeyPlannerStationDetail
                            vc.newupwordtrip = newApiData?.first?.upwardTrip as?[JourneyPlannerModel]
                            
                            vc.downwardTripReturn = newApiData?.first?.downwardTrip?[0].transitPaths as? [TransitPaths]
                            vc.downwardJourneyReturn = newApiData?.first?.downwardTrip?[0].journeyPlannerStationDetail
                        ...
                        if newApiData?.first?.upwardTrip?[0].transitPaths?.count ?? 0 <= 0{
                            self.showAlertViewWithMessage("APPTITLE".LocalizedString, message: "txtNoTrip".LocalizedString)
                        }else if newApiData?.first?.downwardTrip?[0].transitPaths?.count ?? 0 <= 0{
                            self.showAlertViewWithMessage("APPTITLE".LocalizedString, message: "txtNoTrip".LocalizedString)
                        }
                        else{
                            let vc =  UIStoryboard.UpwordNewJourneyVC()
