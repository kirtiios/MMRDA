...
class GrivanceViewModel {
    ...
    func submitGrinacne(complationHandler:@escaping ((Bool)->Void?)){
        ...
        if self.objCategory == nil {
            ...
            var strfileName = "\(Date().timeIntervalSince1970).jpg"
            if isPDFFile {
                strfileName = fileName
            }
            
            SVProgressHUD.show()
            ApiRequest.shared.requestPostMethodForMultipart(strurl: apiName.grivanceSubmit, fileName: strfileName, fileParam: "strIDProof", fileData: data, params: param, showProgress: false) { suces, param in
