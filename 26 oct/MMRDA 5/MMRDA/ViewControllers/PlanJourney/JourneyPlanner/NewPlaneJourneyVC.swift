...
class NewPlaneJourneyVC: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        txtTime.addInputViewDatePickerForTime(target: self, selector: #selector(doneButtonPressed), datePickerMode: .time, Messageget: "txt_error_time".LocalizedString)
...
//            self.txtTime.text = dateFormatter.string(from: datePicker.date)
            ...
            let selectedDate = dateFormatter.date(from: self.txtDate.text ?? "")

            // Use Calendar to compare only date components
            let calendar = Calendar.current
            let currentDateComponents = calendar.dateComponents([.year, .month, .day], from: currentDate)
            let selectedDateComponents = calendar.dateComponents([.year, .month, .day], from: selectedDate ?? Date())
                ...
                newApiviewModel.getReturnJourneyPlanner(param: param) { data in
                    ...
                    if self.isReturnJourney == false{
                        if data.data?[0].upwardTrip?[0].first?.transitPaths?.count ?? 0 <= 0{
                            ...
                            vc.arrUpWard = data.data?[0].upwardTrip?.first as?[JourneyPlannerModel]
                        ...
                        if data.data?[0].downwardTrip?.first?.count ?? 0 <= 0{
                           ...
                           // vc.upwardRoundJourney = data.data?[0].upwardTrip?[0].first?.journeyPlannerStationDetail
                            vc.arrUpward = data.data?[0].upwardTrip?.first as?[JourneyPlannerModel]
                            vc.arrDownWard = data.data?[0].downwardTrip?.first as?[JourneyPlannerModel]
...
//                }
            ...
            vc.completionBlock = { obj in
                ...
                if sender == self.btnFrom {
                    self.objFrom = obj
                    self.QuickFromstationName = obj.locationname
//                    self.btnFrom.setTitle(obj.locationname, for: .normal)
                }else if sender == self.btnTo {
                    self.objTo = obj
                    self.QuickToStationName = obj.locationname
//                    self.btnTo.setTitle(obj.locationname, for: .normal)
                }
...
extension UITextField {
    ...
    func addInputViewDatePickerForTime(target: Any, selector: Selector , datePickerMode:UIDatePicker.Mode,Messageget:String) {
...
//        datePicker.locale = Locale(identifier: "en_US_POSIX") // Change the identifier if needed
    ...
    @objc func datePickerValueChanged(_ sender: UIDatePicker) {
        ...
        if selectedComponents == currentComponents {
