...
class PlanjourneyRouetDetailsVC: BaseVC {
    ...
    var indexrow = -1
    var objparent:SuggestedItineraryVC?
    var objparent_return:UpwordNewJourneyVC?
    ...
    override func viewWillAppear(_ animated: Bool) {
        if isfromSuggestedItineratyVC == true{
...
//            self.navigationController?.popToViewController(ofClass: JourneySearchVC.self)
    ...
    func refreshandAddMarker(){
        ...
        if let obj = objJourney?.journeyPlannerStationDetail {
            ...
            if let etaString = arr.first?.etaNode1, let etaDate = dateFormatter.date(from: etaString) {
                dateFormatter.dateFormat = "HH:mm a"
                let formattedETA = dateFormatter.string(from: etaDate)
               
                marker.snippet =  "station_name".LocalizedString + ": \(arr.first?.fromStationName ?? "")\n" + "txtStationNo".LocalizedString + ": \(arr.first?.edgeSrNo ?? 0)\n" +
                "txtETA".LocalizedString+": \(formattedETA)"
            } else {
                marker.snippet =  "station_name".LocalizedString+": \(arr.first?.fromStationName ?? "")\n" + "txtStationNo".LocalizedString + ": \(arr.first?.edgeSrNo ?? 0)\n" +
                "txtETA".LocalizedString+": \(arr.first?.etaNode2 ?? "")"
            }
            marker.snippet = "txtStartLocation".LocalizedString+": \(objStation?.from_locationname ?? "")\n"
        ...
        if arr.count > 0 {
            ...
            if let etaString = arr.last?.etaNode2, let etaDate = dateFormatter.date(from: etaString) {
                dateFormatter.dateFormat = "hh:mm a"
                let formattedETA = dateFormatter.string(from: etaDate)
               
                marker.snippet =  "station_name".LocalizedString + ": \(arr.last?.fromStationName ?? "")\n" + "txtStationNo".LocalizedString + ": \(arr.last?.edgeSrNo ?? 0)\n" +
                "txtETA".LocalizedString+": \(formattedETA)"
            } else {
                marker.snippet =  "station_name".LocalizedString+": \(arr.last?.fromStationName ?? "")\n" + "txtStationNo".LocalizedString + ": \(arr.last?.edgeSrNo ?? 0)\n" +
                "txtETA".LocalizedString+": \(arr.last?.etaNode2 ?? "")"
            }
...
//            marker.snippet = arr.last?.toStationName
        ...
        if let obj = objJourney?.journeyPlannerStationDetail?.startingMYBYKPath{//objJourney?.journeyPlannerStationDetail?.startingMYBYKPath {
            ...
            marker.map = mapView
            marker.snippet =  "txtMyBykHubName".LocalizedString+": \(obj.strFromMYBYKStationName ?? "")\n" + "txtMybykETA".LocalizedString + ": \(obj.strFromHubArrival ?? "")\n"
//            marker.snippet = obj.strFromMYBYKStationName
        ...
        if let obj = objJourney?.journeyPlannerStationDetail?.startingMYBYKPath {
            ...
            marker.icon = UIImage(named:"MapMYBike")
            marker.map = mapView
            marker.snippet =  "txtMyBykHubName".LocalizedString+": \(obj.strToMYBYKStationName ?? "")\n" + "txtMybykETA".LocalizedString + ": \(obj.strToHubArrival ?? "")\n"
...
//            marker.snippet = obj.strToMYBYKStationName
        ...
        if objJourney?.journeyPlannerStationDetail?.modeOfFromStationTravel?.lowercased() == "a"{
            ...
            marker.map = mapView
            marker.snippet = "txtStartLocation".LocalizedString+": \(objJourney?.journeyPlannerStationDetail?.strFromStationName ?? "")\n"
//            marker.snippet = objJourney?.journeyPlannerStationDetail?.strFromStationName
        ...
        if objJourney?.journeyPlannerStationDetail?.modeOfToStationTravel?.lowercased() == "a"{
            ...
            marker.icon = UIImage(named:"Rickshaw")
            marker.map = mapView
            marker.snippet = "txtEndLocation".LocalizedString+": \(objJourney?.journeyPlannerStationDetail?.strToStationName ?? "")\n"
//            marker.snippet = objJourney?.journeyPlannerStationDetail?.strToStationName
        ...
        if let obj = objJourney?.journeyPlannerStationDetail {
            ...
            marker.map = mapView
            marker.snippet = "txtEndLocation".LocalizedString+": \(objStation?.to_locationname ?? "")\n"
...
//        }
    ...
    @IBAction func actiobBookNow(_ sender: Any) {
        if objJourney?.journeyPlannerStationDetail?.isBusOnly == true{
...
extension PlanjourneyRouetDetailsVC :UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            ...
            if let strFromStationName = objJourney?.journeyPlannerStationDetail?.stationArrival {
                let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
                
                if let date = dateFormatter.date(from: strFromStationName) {
...
////            
            ...
            if let obj = objJourney?.journeyPlannerStationDetail?.endingMYBYKPath {
                ...
                if let etaNode1 = obj.strFromHubArrival {
                    let formattedTime = formatDateString(etaNode1, from: "yyyy-MM-dd HH:mm:ss", to: "hh:mm a")
                    
                    if let formattedTime = formattedTime {
                        cell.lblTOTime1.text = formattedTime
                    } else {
...
//                cell.lblTOTime1.text =  (obj.strFromHubArrival ?? "").getCurrentDatewithDash().toString(withFormat:"hh:mm a")
                if let etaNode1 = obj.strToHubArrival {
                    let formattedTime = formatDateString(etaNode1, from: "yyyy-MM-dd HH:mm:ss", to: "hh:mm a")
                    
                    if let formattedTime = formattedTime {
                        cell.lblTOTime2.text = formattedTime
                    } else {
                        // Handle the case where the date string is not valid
                        print("Invalid date string")
                    }
...
//                cell.lblTOTime2.text =  (obj.strToHubArrival ?? "").getCurrentDatewithDash().toString(withFormat:"hh:mm a")
            ...
            if let obj = objJourney?.journeyPlannerStationDetail?.startingMYBYKPath {
               ...
               // cell.lblFromTime1.text =  (obj.strFromHubArrival ?? "").getCurrentDatewithDash().toString(withFormat:"hh:mm a")
                if let etaNode1 = obj.strFromHubArrival {
                    let formattedTime = formatDateString(etaNode1, from: "yyyy-MM-dd HH:mm:ss", to: "hh:mm a")
                    if formattedTime == "" ||  formattedTime == nil{
                        cell.lblFromTime1.isHidden = true
                    }else{
                        
                        if let formattedTime = formattedTime {
                            cell.lblFromTime1.text = formattedTime
                        } else {
                            // Handle the case where the date string is not valid
                            print("Invalid date string")
                        }
...
//                cell.lblFromTime2.text =  (obj.strToHubArrival ?? "").getCurrentDatewithDash().toString(withFormat:"hh:mm a")
                if let etaNode1 = obj.strToHubArrival {
                    ...
                    if formattedTime == "" ||  formattedTime == nil{
                        cell.lblFromTime2.isHidden = true
                    }else{
                        if let formattedTime = formattedTime {
                            cell.lblFromTime2.text = formattedTime
                        } else {
                            // Handle the case where the date string is not valid
                            print("Invalid date string")
                        }
            ...
            if let etaNode1 = arrOriginal.last?.etaNode2 {
                let formattedTime = formatDateString(etaNode1, from: "yyyy-MM-dd HH:mm:ss", to: "hh:mm a")
                
                if let formattedTime = formattedTime {
                    cell.lblToTime.text = formattedTime
                } else {
                    // Handle the case where the date string is not valid
                    print("Invalid date string")
                }
...
//            cell.lblMainToStation.text = objStation?.to_locationname
            ...
            cell.completionBlockData = {
                if arrNew.count > indexPath.row {
                    if let etaNode1 = arrNew[indexPath.row].etaNode1 {
                        let dateFormatter = DateFormatter()
                        dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
                        if let date = dateFormatter.date(from: etaNode1) {
                            let timeFormatter = DateFormatter()
                            timeFormatter.dateFormat = "HH:mm a"
                            let formattedTime = timeFormatter.string(from: date)
//                            cell.lbltime.text = formattedTime
                ...
                if let obj = obj {
                    ...
                    if let firstPresented = UIStoryboard.ReminderVC() {
                        ...
                        firstPresented.completionNotifyAction = { indexpath,obj,count,isSave in
                            ...
                            if self.isfromSuggestedItineratyVC {
                                if let model = self.objJourney {
                                    self.objparent?.arrUpWard?[self.indexrow] = model
                                }
                            }
                            if self.isfromUpwoedNewJourneyVc{
                                if let model = self.objJourney {
                                    self.objparent_return?.arrUpward?[self.indexrow] = model
                                    self.objparent_return?.arrDownWard?[self.indexrow] = model
                                }
                            }
...
extension PlanjourneyRouetDetailsVC {
    ...
    func formatDateString(_ dateString: String, from sourceFormat: String, to destinationFormat: String) -> String? {
        let dateFormatter = DateFormatter()
