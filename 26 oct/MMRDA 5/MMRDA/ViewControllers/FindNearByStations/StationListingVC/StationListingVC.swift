...
class StationListingVC: BaseVC {
    ...
    var transportType:transPortType = .Metro
    ...
    @IBAction func actionFavourites(_ sender: Any) {
        self.objViewModel.saveFavouriteStation(stationid:"\(objStation?.stationid ?? 0)", transportMode:transportType.rawValue)
    }
}
...
extension StationListingVC {
    ...
    func getStationList(time:Int){
       ...
       // param["tmScheduleTime"] = "00:00"
        ...
        param["TransportType"] = transportType.rawValue
...
extension StationListingVC :UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if transportType == transPortType.Metro {
            cell.btnRoute .setTitle(objdata?.strMetroLineNo ?? "NA", for: .normal)
        }else {
           // cell.btnRoute.isHidden = true
           cell.btnRoute .setTitle("NA", for: .normal)
        }
