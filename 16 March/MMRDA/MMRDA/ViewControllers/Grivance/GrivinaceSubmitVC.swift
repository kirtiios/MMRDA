...
//                self.lblFileName.text = self.randomString(8) + ".jpg"
                if let image = docName as? UIImage {
                    // docName is an image
                    self.btnUploadFile.setImage(image, for: .normal)
                    self.lblFileName.text = self.randomString(8) + ".jpg"
                }
...
//                   }
                    ...
                    self.btnUploadFile.setBackgroundImage(UIImage(named: "docs"), for: .normal)
                   self.btnUploadFile.setImage(UIImage(named: ""), for: .normal)
   ...
   private func initialize(){
       ...
       objViewModel.fileName = ""
       objViewModel.data = nil
       objViewModel.strDescription = nil
       btnUploadFile.setImage(UIImage(named: ""), for: .normal)
       self.btnUploadFile.setBackgroundImage(UIImage(named: "upload"), for: .normal)
     
    }
    @IBAction func actionSubmit(_ sender: UIButton) {
        
        self.objViewModel.isPDFFile = false
        ...
        if img?.pngData() != UIImage(named:"upload")?.pngData() {
            ...
            self.objViewModel.fileName = self.lblFileName.text ?? ""
        }
        else {
            data = img?.pngData()
