...
//        lblAmount.text = "\("rs".LocalizedString).\(arrHistory.first?.totaL_FARE ?? 0)"
        lblAmount.text = "Rs.\((arrHistory.first?.total_Amount ?? 0) * arrHistory.count)"
        lblConfirmDay.text = "\(strPaymentStatus)," + "txtToday".LocalizedString
