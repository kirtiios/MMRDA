...
//    var arrImage = ["FareCalculator","PlanYourJourney","FindNearByStation","MyTicket","MyPass","SmartCard"]
    var arrImage = ["ticket_booking","plan_journey","nearby","my_ticket","my_pass","smart_card"]
    func initialize(){
        lblFullName.text = "welcomeback".localized()  + " "  +  (Helper.shared.objloginData?.strFullName ?? "")
        btnHelpLine .setTitle("helpline".localized(), for:.normal)
        self.collectionView.reloadData()
    }
    @IBOutlet var slideshow: ImageSlideshow!
    
    @IBOutlet var bottomSlideshow: ImageSlideshow!
...
//        slideshow.addGestureRecognizer(recognizer)
        ...
        bottomSlideshow.slideshowInterval = 5.0
        bottomSlideshow.pageIndicatorPosition = .init(horizontal: .center, vertical: .under)
        bottomSlideshow.contentScaleMode = UIViewContentMode.scaleToFill

        bottomSlideshow.pageIndicator = nil
        bottomSlideshow.activityIndicator = DefaultActivityIndicator()
        bottomSlideshow.delegate = self
        bottomSlideshow.setImageInputs([
            ImageSource(image:UIImage(named: "banner1")!),
            ImageSource(image:UIImage(named: "banner2")!)])
...
//        cell.contentView.backgroundColor = Colors.APP_Theme_color.value
        cell.lnlMenuName.text = arrName[indexPath.row].localized()
        cell.lblcomingsoon.text = "coming_soon".localized()
        cell.imgMenu.image = UIImage(named: arrImage[indexPath.row])
        cell.viewComingSoon.isHidden = true
        if DashboardMenus(rawValue: indexPath.row) == .SmartCard ||  DashboardMenus(rawValue: indexPath.row) == . Mypass {
            cell.viewComingSoon.isHidden = false
...
//    }
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
            ...
            let cellWidth = (screenWidth - 20) / 3 // Adjust the spacing as per your requirement
            return CGSize(width: cellWidth, height: 150)//cellWidth + 30)
        }
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets
    {
       let sectionInset = UIEdgeInsets(top:0, left:5, bottom:5, right:5)
