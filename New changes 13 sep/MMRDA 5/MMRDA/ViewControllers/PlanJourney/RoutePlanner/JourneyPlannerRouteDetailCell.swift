...
class JourneyPlannerRouteDetailCell: UITableViewCell {
    ...
    var completionBlockNotify:((TransitPaths?,Int) ->(Void))?
...
//        return tblView.isHidden ? 0 : arrRoutePaths.count
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        print("tableview Cell stationname:- \(objdata.fromStationName)")
        ...
        let formattedDateArrival = formattedDateString(from: objdata.etaNode1 ?? "", inputFormat: "yyyy-MM-dd HH:mm:ss", outputFormat: "hh:mm a")
       
        cell.lbltime.text = formattedDateArrival//(objdata.etaNode1 ?? "").getCurrentDatewithDash().toString(withFormat:"hh:mm a")
        ...
        cell.completionBlockNotify = { index  in
            if let index = index {
                self.completionBlockNotify?(self.arrRoutePaths[index], 0)
            }
            if self.arrRoutePaths.count > 2{
                print("button show")
            }else{
                print("button hide")
            }
