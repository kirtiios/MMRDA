...
class ReminderVC: UIViewController {
 ...
 //   var completionNotifyRemove:((IndexPath?,Any?,Int?,Bool) ->(Void))?
    var completionNotifyAction:((IndexPath?,Any?,Int?,Bool) ->(Void))?
...
//        intType =2 plan journey
        ...
        if btnSave.titleLabel?.text == "lbl_save".LocalizedString{
              objViewModel.saveNotifyAlarm(param: param) { sucess,count in
                  self.dismiss(animated: true) {
...
//                      }
                      ...
                      self.completionNotifyAction?(self.indexpath,self.obj,1,true)
...
//                      })
            ...
            objViewModel.RemoveNotify(param: param) { Success,count in
                self.dismiss(animated: true) {
...
//
//                    }
                    self.completionNotifyAction?(self.indexpath,self.obj,count,false)
